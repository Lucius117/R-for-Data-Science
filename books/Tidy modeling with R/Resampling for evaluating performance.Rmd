...
```{r}
mc_cv(ames_train, prop = 9/10, times = 50)
```
...
time_slices <- 
  tibble(x = 1:365) %>% 
  rolling_origin(initial = 6 * 30, # The number of samples used for analysis/modeling in the initial resample
                 assess = 30, # The number of samples used for each assessment resample
                 #skip = 29, 
                 cumulative = FALSE)
time_slices
```
