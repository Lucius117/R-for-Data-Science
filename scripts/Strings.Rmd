...
```{r}
# each string's length in a character vector
...
But if “.” matches any character, how do you match the character “.”? You need to use an “escape” to tell the regular expression you want to match it exactly, not use its special behaviour.
...
str_view(c("abc","a.c",".?a-c^b\\d][","a*c"),"\\.[?].[-]c\\^b\\\\d[]]\\[")
```
...
y default these matches are “greedy”: they will match the longest string possible. You can make them “lazy”, matching the shortest string possible by putting a ? after them. This is an advanced feature of regular expressions, but it’s useful to know that it exists:
...
The results are identical, but I think the first approach is significantly easier to understand. If your regular expression gets overly complicated:

1. try breaking it up into smaller pieces, 

2. giving each piece a name, 

3. combining the pieces with logical operations.
...
```{r}
fields <- c("Name: Hadley","Country: NZ","Age: 35: 36")
fields %>% 
...
# Other use of regular expressions
